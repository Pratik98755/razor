...
from api import get_merchant_activities
...
def activity_page(user):
    ...
    activities = get_merchant_activities(user["user_id"])
    ...
    with st.expander("🔎 Filters", expanded=True):
        ...
        with col1:
            search_action = st.text_input(
                "Search Action",
                placeholder="e.g. product added, product edited...",
                key="merchant_activity_search_action",
            )

        with col2:
            start_date = st.date_input(
                "From", value=None, key="merchant_activity_start_date"
            )

        with col3:
            end_date = st.date_input("To", value=None, key="merchant_activity_end_date")

        if st.button("Clear Filters", key="clear_merchant_activity_filters"):
            st.session_state.merchant_activity_search_action = ""
            st.session_state.merchant_activity_start_date = None
            st.session_state.merchant_activity_end_date = None
            st.rerun()
